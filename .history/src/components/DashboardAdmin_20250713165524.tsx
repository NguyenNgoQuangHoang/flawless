...
import type { RevenueData } from "../redux/slices/revenueSlice";
...
export default function DashboardAdmin() {
	...
	useEffect(() => {
		...
		dispatch(fetchTotalRevenue())
			.then((action) => {
				console.log('Revenue data response:', action);
			})
			.catch((error) => {
	...
	const selectedYearDataRevenue = dataRevenue?.perYear.find(
	...
	const selectedYearDataCustomer = dataCustomer?.perYear.find(
		(y: { year: number }) => y.year === Number(yearCustomer)
	);

	const customerData = selectedYearDataCustomer?.months[monthCustomer] ?? {
		totalCustomer: 0,
		newCustomer: 0,
		returnCustomer: 0,
		cancelCustomer: 0,
				...
				<div className="w-2/3 space-y-4">
						...
						<Card
							...
							value={
								dataCustomer?.loading
									? "Loading..."
									: dataCustomer?.data?.totalCustomerAllYear != null
										? dataCustomer.data.totalCustomerAllYear.toString()
										: "0"
							}
							icon={faUser}
						/>
					...
					<div className="bg-white p-4 rounded-2xl shadow-xl">
						<div className="flex justify-between items-center mb-2">
							...
							>
								{Array.isArray(dataRevenue?.perYear) &&
									dataRevenue.perYear.map(
										(y: { year: number }) => (
						...
						<div className="flex justify-between">
							...
							<div className="flex gap-2">
								...
								<select
									...
									className="text-sm px-2 py-1 rounded-xl bg-gradient-to-r from-green-50 to-emerald-50"
								>
									{Array.isArray(dataCustomer?.perYear) &&
										dataCustomer.perYear.map(
											(item: { year: number }) => (
												<option key={item.year} value={item.year}>
													{item.year}
												</option>

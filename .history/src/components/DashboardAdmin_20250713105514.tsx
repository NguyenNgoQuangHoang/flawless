...
export default function DashboardAdmin() {
	...
	const { data: dataRevenue } = useSelector<RootState, { data: RevenueData | null; loading: boolean; error: string | null }>(
		(state) => state.revenue
	);
	...
	const selectedYearDataRevenue = dataRevenue?.data?.perYear.find(
				...
				<div className="w-2/3 space-y-4">
					...
					<div className="grid grid-cols-2 gap-4">
						<Card
							...
							title="Earnings"
							value={
								dataRevenue?.data?.totalIncomeAllYear
									? formatCurrencyVND(dataRevenue.data.totalIncomeAllYear)
									: "Loading..."
							}
							icon={faDollarSign}
						/>
					...
					<div className="bg-white p-4 rounded-2xl shadow-xl">
						<div className="flex justify-between items-center mb-2">
							...
							>
								{Array.isArray(dataRevenue?.data?.perYear) &&
									dataRevenue.data.perYear.map(
										(y: { year: number }) => (
